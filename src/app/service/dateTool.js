...
app.service('DateTool', function () {
    var ONE_DAY = 1000 * 3600 * 24;

    // month & week's array
    var fullMonthName = ['January', 'February', 'March', 'April', 'May', 'June',
                    'July', 'August', 'September', 'October', 'November', 'December'];
    var fullWeekName = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    ...
    var timeDist = function (date1, date2) {
    ...
    var dateTool =  {
        ...
        format: function (expr, date) {
            ...
            if (arguments.length === 1) {
                ...
                date = new Date(window.parseInt(date) || 0);
            }
         ...
         * @method isLeapYear
         * @param {Date|String} year [input year]
         * @return {boolean}
         * @example
         ...
         * @method lastDay
         * @param {Date|Number} date [inpute dateTime]
         * @return {Date}
         * @example
        ...
        /**
         * Get Timestamp of the Start of Day. description
         * @method startDay
         * @param {Date|Number} date [inpute dateTime]
         * @param {Object} opt [ouput format options]
         * @return {Date}
         * @example
         *      dateTool.startDay(new Date('2000-1-10')); // returns +new Date('2000-1-10 00:00:00')
         */
        startDay: function (date, opt) {
            if (opt && opt.format === 'Y-m-d') {
                return dateTool.format('Y-m-d', +new Date(dateTool.format('Y-m-d', date) + ' 00:00:00'));
            }
            return +new Date(dateTool.format('Y-m-d', date) + ' 00:00:00');
        },

        /**
         * Get Timestamp of the End of Day.
         * @method endDay
         * @param {Date|Number} date [inpute dateTime]
         * @param {Object} opt [ouput format options]
         * @return {Date}
         * @example
         *      dateTool.endDay(new Date('2000-1-10')); // returns +new Date('2000-1-10 23:59:59')
         */
        endDay: function (date, opt) {
            if (opt && opt.format === 'Y-m-d') {
                return dateTool.format('Y-m-d', +new Date(dateTool.format('Y-m-d', date)
                                        + ' 00:00:00') + ONE_DAY - 1000);
            }
            return +new Date(dateTool.format('Y-m-d', date) + ' 00:00:00') + ONE_DAY - 1000;
        },
        ...
        nextDay: function (date) {
            if (arguments.length === 0) {
                return new Date(+new Date() + ONE_DAY);
            }
            return new Date(+date + ONE_DAY);
        },
        ...
        daysBetween: function (start, end) {
            ...
            return parseInt((+end - timeZoneExtTime) / ONE_DAY, 10)
                        - parseInt((+start - timeZoneExtTime) / ONE_DAY, 10);
        },
